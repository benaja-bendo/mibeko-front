...
- **Destination par défaut d'un compte : une seule règle, `features/auth/redirect.ts`.** `defaultRedirectFor()` est partagée par `LoginPage`, `RegisterPage`, `RedirectIfAuthenticated` et `RootRedirect`. Le staff va dans son espace, les autres comptes vérifiés vont sur `/app/library`, et un nouveau compte soumis à vérification reste sur `/auth/verifier-email`. Ne pas réintroduire de redirection locale dans un de ces points.
- **L'inscription, la vérification d'e-mail et la réinitialisation de mot de passe vivent ici**, pas sur `mibeko.fr` dont la règle absolue interdit tout compte : `/auth/register`, `/auth/verifier-email`, `/auth/mot-de-passe-oublie`, `/auth/reinitialiser`. La vérification conserve l'intention `next=assistant`, permet le renvoi du lien et recontrôle `/me` au retour dans l'onglet. La réinitialisation se fait par **code à 6 chiffres envoyé par e-mail** (15 min), pas par lien. ⚠️ `PasswordResetCodeNotification` est `ShouldQueue` : sans worker de file, aucun code ne part et l'API répond quand même 200 (anti-énumération volontaire) — en local, `QUEUE_CONNECTION=sync` ou un `queue:work` lancé à la main.
...
## Marque
Le dashboard utilise par défaut **« Mibeko Classique »** (`DEFAULT_THEME_ID = 'mibeko-classic'`) : fond clair, accent forêt, cohérent avec le site et l'app mobile. **« Lex Gold »** reste disponible comme thème sombre optionnel. Une préférence déjà enregistrée reste toujours prioritaire sur le défaut.
