...
async function pyFetch<T>(path: string, options?: RequestInit): Promise<T> {
  const res = await fetch(`${BASE}${path}`, {
    headers: options?.body instanceof FormData
      ? { 'Accept': 'application/json' }  // Let browser set Content-Type for FormData
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || err.detail || `Python API error ${res.status}`);
  }
...
export const uploadDocument = async (formData: FormData): Promise<UploadResponse> => {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ message: res.statusText }));
    throw new Error(err.message || err.detail || `Upload error ${res.status}`);
  }
...
export const parseDocument = async (
  ...
  const res = await fetch(`${BASE}/documents/${docId}/parse`, {
    method: 'POST',
    body: fd,
    headers: { 'Accept': 'application/json' },
  });
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ message: res.statusText }));
    throw new Error(err.message || err.detail || `Parse error ${res.status}`);
  }
