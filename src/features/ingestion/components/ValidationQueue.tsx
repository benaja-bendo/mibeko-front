...
/** Mini-pipeline : disponibilité des artefacts et volume parsé. */
function PipelineCell({ doc }: { doc: PythonDocumentSummary }) {
  ...
  return (
    <div className="flex items-center gap-1.5 text-[10px] font-mono whitespace-nowrap">
      <span className="text-t2">PDF</span>
      <span className="text-t4">→</span>
      <span className={doc.has_md ? 'text-green' : 'text-t4'}>MD</span>
      <span className={doc.has_json ? 'text-blue' : 'text-t4'}>JSON</span>
      <span className="text-t4">→</span>
      {isRunning ? (
        ...
        <span className="text-gold">{doc.nb_articles} art.</span>
      ) : (
        <span className="text-t4">0 art.</span>
      )}
    </div>
  );
}
