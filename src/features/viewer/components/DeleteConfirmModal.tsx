      ...
      <DialogContent className="sm:max-w-[400px] border-red/20">
        <DialogHeader className="items-center sm:items-start">
          ...
          <DialogDescription className="text-center sm:text-left pt-2">
            Êtes-vous sûr de vouloir supprimer <strong>{deleteNode.type === 'ARTICLE' ? 'l\'article' : deleteNode.type === 'DOCUMENT' ? 'le document' : 'la division'} {getDeleteLabel()}</strong> ?
            <br />
        ...
        <DialogFooter className="pt-6 sm:space-x-3 gap-2 sm:gap-0">
          ...
          <Button 
            ...
            disabled={deleteNodeMutation.isPending || deleteArticleMutation.isPending || deleteDocumentMutation.isPending}
            className="flex-1 sm:flex-none bg-red text-white hover:bg-red/90"
          >
            <Trash2 className="w-4 h-4 mr-2" />
            {deleteNodeMutation.isPending || deleteArticleMutation.isPending || deleteDocumentMutation.isPending ? 'Suppression...' : 'Supprimer définitivement'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
